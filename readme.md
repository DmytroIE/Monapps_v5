...
* https://github.com/DmytroIE/MonappsAzure A small application that is also subscribed to the `procdata` topic but is interested only in new ***datafeed readings***. When new readings are created, it fetches them via an HTTP request, repacks, and sends them to Azure IoT Hub. When a new ***datafeed*** is created in the Monapps instance, the program adds it to the database automatically.